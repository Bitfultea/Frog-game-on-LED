...
static uint64_t lane_data[3] = {
		...
		0b0011110001100011000011110000111100101110011000011100101001100101,
		0b0101000101010001010101000101000100101001001001001001010001100111,
};
...
static uint32_t log_data[2] = {
		...
		0b11100110111101100001110110011100,
		0b11000110111001100001100110011000,
		0b10101010101001100101100100011001,
};
...
PixelColour vehicle_colours[5] = { COLOUR_RED, COLOUR_YELLOW, COLOUR_RED ,COLOUR_YELLOW, COLOUR_YELLOW}; // by lane
...
static void redraw_traffic_lane(uint8_t lane) {
	...
	uint8_t colour_lane;
	uint8_t patterns_lane;
	colour_lane = lane + levels - 1;
	patterns_lane = lane + levels - 1;
	for(i=0; i<=15; i++) {
		if((lane_data[patterns_lane] >> bit_position) & 1) {
			row_display_data[i] = vehicle_colours[colour_lane];
			} else {
...
static void redraw_river_channel(uint8_t channel) {
	...
	uint8_t patterns_channel;
	patterns_channel = channel + levels - 1;
	for(i=0; i<=15; i++) {
		if((log_data[patterns_channel] >> bit_position) & 1) {
